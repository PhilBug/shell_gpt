...
class AnthropicProvider:
    ...
    def __init__(self) -> None:
        ...
        # Anthropic has its own base URL setting; never reuse the OpenAI
        # API_BASE_URL here (e.g. a Groq/OpenAI-compatible endpoint would
        # otherwise produce requests like .../openai/v1/v1/messages).
        base_url_cfg = cfg.get("ANTHROPIC_BASE_URL")
    ...
    def get_completion(
        ...
        *,
        ...
        # Anthropic rejects requests that specify both `temperature` and
        # `top_p` (e.g. claude-haiku-4-5 returns HTTP 400). The CLI defaults
        # top_p to 1.0, which is a no-op, so only forward it when the user
        # actually narrowed the nucleus (top_p < 1.0).
        if top_p < 1.0:
            kwargs["top_p"] = top_p
        ...
        if tools:
            kwargs["tools"] = tools
        if cfg.get("ANTHROPIC_THINKING") == "disabled":
    ...
    def _start_stream(self, kwargs: Dict[str, Any]) -> Any:
        from anthropic import BadRequestError

        try:
            return self._client.messages.stream(**kwargs).__enter__()
        except BadRequestError as error:
            message = str(error).lower()
            # Both are cases where a model rejects a field this CLI sends
            # unconditionally (or that the user opted into): sampling params
            # are deprecated/rejected outright on Claude Opus 4.7+/Sonnet 5+,
            # and "disabled" thinking may not be a recognized field on older
            # models. Only strip fields the error message actually names as
            # rejected (not, say, a genuine out-of-range value error), then
            ...
            is_rejection = any(word in message for word in rejection_words)
            droppable = {
                "temperature": ("temperature", "top_p", "top_k"),
                "top_p": ("temperature", "top_p", "top_k"),
                "top_k": ("temperature", "top_p", "top_k"),
                "thinking": ("thinking",),
            }
            to_drop = {
                field
                for key, fields in droppable.items()
                if is_rejection and key in message and key in kwargs
                for field in fields
                if field in kwargs
            }
            if to_drop:
                retry_kwargs = {k: v for k, v in kwargs.items() if k not in to_drop}
                return self._client.messages.stream(**retry_kwargs).__enter__()
            raise
    ...
    def parse_stream(
        ...
        stream = response.__enter__() if hasattr(response, "__enter__") else response
        try:
            for event in stream:
                ...
                if etype == "content_block_start":
                    ...
                    if dtype == "text_delta":
                        ...
                        tool_inputs[event.index] += getattr(delta, "partial_json", "")
